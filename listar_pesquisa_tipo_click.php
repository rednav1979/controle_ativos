...
<span id='topo'></span>
    <section class="hero is-success is-fullheight">
                ...
                <br><br><br><br>
					...
					<br><br>
					<p align=right><font size=2 color=blue><a href =listar.php>VOLTAR?</a></font>
                    <div class="box">
					<p align=left>
...
<select name="colaborador_procura"   autofocus="" >									
  ...
  <?php 
   include "sql_t.i.php";//conexão com o banco de dados   
   @mysql_select_db($db);//selecione o banco de dados 
           $sqltudo = mysql_query(" select tipo_equipamento  from crtl_patrimonio where D_E_L_E_T_E IS NULL group by tipo_equipamento;")  or die(mysql_error());
           $colunas = mysql_num_rows($sqltudo);		
           for($j = 0; $j < $colunas; $j++){/*caso no mesmo dia tenha mais eventos continua imprimindo */                   
		   $id = @mysql_result($sqltudo, $j, "id");           
...
<td><input type="submit" value="Pesquisar" /><input type="hidden" name="done"  value="" /></td>
   ...
   @mysql_select_db($db);//selecione o banco de dados
	...
	$sqltudo = mysql_query("select  * FROM crtl_patrimonio  where D_E_L_E_T_E IS NULL  and tipo_equipamento='$colaborador_retorno' order by colaborador ")  or die(mysql_error());
