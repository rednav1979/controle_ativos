...
<form name="form2" action="listar_pesquisa_obra_tipo.php" method="post">
...
<option>SELECIONE</option>
  <?php 
   include "sql_t.i.php";//conexão com o banco de dados   
   @mysql_select_db($db);//selecione o banco de dados 
           $sqltudo = mysql_query(" select tipo_equipamento  from crtl_patrimonio  where obra = ('$colaborador_retorno')and D_E_L_E_T_E IS NULL group by tipo_equipamento;")  or die(mysql_error());
           $colunas = mysql_num_rows($sqltudo);		
           for($j = 0; $j < $colunas; $j++){/*caso no mesmo dia tenha mais eventos continua imprimindo */                   
		   $id = @mysql_result($sqltudo, $j, "id");           
...
<p align=lef> OBRAS: <font color=red><?php echo $colaborador_retorno;?></p></font>	
   ...
   @mysql_select_db($db);//selecione o banco de dados
	...
	$colaborador_retorno = $_POST['colaborador_procura'];
	$sqltudo = mysql_query("select  * FROM crtl_patrimonio  where obra like UPPER('$colaborador_retorno') and D_E_L_E_T_E IS NULL order by departamento ")  or die(mysql_error());
	  ...
	  //COLOCA O ICONE   DO EQUIPAMENTO QUE CONSTA NA TABLEA CONTROLE_PATRIMONIO NO CAMPO ICONE  
	   ...
	   include "sql_t.i.php";//conexão com o banco de dados   
	   @mysql_select_db($db);//selecione o banco de dados
			  $sqltudo = mysql_query("select count(*)tipo_equipamento,tipo_equipamento  as type from crtl_patrimonio  where obra like UPPER('$colaborador_retorno') and D_E_L_E_T_E IS NULL  group by tipo_equipamento")  or die(mysql_error());
			  $colunas = mysql_num_rows($sqltudo);	   
			  for($j = 0; $j < $colunas; $j++){/*caso no mesmo dia tenha mais eventos continua imprimindo */
			  $id = @mysql_result($sqltudo, $j, "id");/*pegando os valores do banco referente ao evento*/           
	...
	<script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ...
                ['class Name','Students'],
                <?php
                $query = " select tipo_equipamento  as jjj,count(tipo_equipamento) as lll from crtl_patrimonio where obra=('$colaborador_retorno') and D_E_L_E_T_E IS NULL group by tipo_equipamento;";
