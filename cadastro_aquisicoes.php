 ...
 <script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js'></script><script  src="./novo_menu.js"></script>
    ...
    <section class="hero is-success is-fullheight">
                ...
                <div class="column is-4 is-offset-4">
                    ...
                    <h1 class="title has-text-grey">CADASTRO DE AQUISIÇÕES </h1><center>
					<h2 class="title has-text-grey"><font size=4>[Tecnologia da Informação]</font></h2>                   
					
                    <div class="box">
                        <form name="form1" action="cadastro_aquisicoes.php" method="POST">
						<?php
if(isset($erro)){
...
?>	
   ...
   @mysql_select_db($db);//selecione o banco de dados
	...
	$id = $_GET['id'];
           $sqltudo = mysql_query("SELECT * FROM crtl_patrimonio_aquisicoes ORDER BY id ")  or die(mysql_error());
           $colunas = mysql_num_rows($sqltudo);
	   print'<br>';		
	   print'<br>';		  
           for($j = 0; $j < $colunas; $j++){/*caso no mesmo dia tenha mais eventos continua imprimindo */
...
?>
   ...
   include "sql_t.i.php";//conexão com o banco de dados   
   @mysql_select_db($db);//selecione o banco de dados
           $sqltudo = mysql_query("select count(*)tipo_equipamento,tipo_equipamento  as type from crtl_patrimonio_aquisicoes group by tipo_equipamento")  or die(mysql_error());
           $colunas = mysql_num_rows($sqltudo);	   
           for($j = 0; $j < $colunas; $j++){/*caso no mesmo dia tenha mais eventos continua imprimindo */
           $id = @mysql_result($sqltudo, $j, "id");/*pegando os valores do banco referente ao evento*/           
		   ...
		   print '<td>'.$nome.':</td>';	
		   print '<a href=listar_pesquisa_tipo_click_aquisicoes.php?tipo_equipamento='.$nome.'>'.$type.'</a>';
		   print '<br>';	
           }
		   ?>
                    </div>
