...
?>
<?php
	$obra_retorno = $_POST['colaborador_procura3'];
	$tipo_equipamento_retorno = $_POST['salva'];
...
<p align=lef> TIPO_EQUIPAMENTO: <font color=red><?php echo $tipo_equipamento_retorno;?></p></font>	
<p align=lef> OBRA: <font color=red><?php echo $obra_retorno;?></p></font>	
   ...
   @mysql_select_db($db);//selecione o banco de dados	
	$sqltudo = mysql_query("select  * FROM crtl_patrimonio  where tipo_equipamento = ('$tipo_equipamento_retorno') and obra = ('$obra_retorno') and D_E_L_E_T_E IS NULL order by colaborador ")  or die(mysql_error());
	   ...
	   include "sql_t.i.php";//conexão com o banco de dados   
	   @mysql_select_db($db);//selecione o banco de dados
			  $sqltudo = mysql_query("select count(*)tipo_equipamento,tipo_equipamento  as type from crtl_patrimonio  where tipo_equipamento = ('$tipo_equipamento_retorno') and obra = ('$obra_retorno') and D_E_L_E_T_E IS NULL group by tipo_equipamento")  or die(mysql_error());
			  $colunas = mysql_num_rows($sqltudo);	   
			  for($j = 0; $j < $colunas; $j++){/*caso no mesmo dia tenha mais eventos continua imprimindo */
			  $id = @mysql_result($sqltudo, $j, "id");/*pegando os valores do banco referente ao evento*/           
